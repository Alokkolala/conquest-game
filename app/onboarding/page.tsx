...
export default function OnboardingPage() {
  ...
  const steps = [
    ...
    <StepHouseName key={1} name={houseName} onChange={setHouseName} displayName={displayName} onNext={() => setStep(2)} />,
    <StepBannerColor key={2} color={color} onChange={setColor} displayName={displayName} onNext={() => setStep(3)} />,
    <StepStartingRegion key={3} selected={region} onSelect={setRegion} onNext={() => setStep(4)} />,
    <StepReady key={4} displayName={displayName} color={color} onEnter={() => router.push('/')} />,
  ]
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 1, textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: 'var(--serif)', fontSize: 56, fontStyle: 'italic', letterSpacing: '-0.025em', lineHeight: 0.9, color: '#f4f1ea', marginBottom: 20 }}>
          Conquest
        </div>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '0.3em', textTransform: 'uppercase', color: 'rgba(244,241,234,0.3)' }}>
          Territory Chess · World Atlas
        </div>
      ...
      <div style={{ width: '100%', zIndex: 1 }}>
        <StepDots total={4} current={0} dark />
  ...
  return (
    <div style={{ minHeight: '100dvh', width: '100%', maxWidth: 390, margin: '0 auto', background: 'var(--bg)', display: 'flex', flexDirection: 'column', padding: '60px 24px env(safe-area-inset-bottom, 40px)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 10 }}>Step 1 of 4</div>
        ...
        <p style={{ fontFamily: 'var(--sans)', fontSize: 14, color: 'var(--muted)', lineHeight: 1.5, marginBottom: 36 }}>
          Your house name is your identity on the map. Choose wisely — rulers are remembered.
        </p>

        {/* Live preview */}
        <div style={{ padding: '20px 20px', background: '#fff', border: '0.5px solid var(--line)', borderRadius: 18, marginBottom: 24, minHeight: 88, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          <div style={{ fontFamily: 'var(--serif)', fontSize: 28, fontStyle: 'italic', letterSpacing: '-0.02em', lineHeight: 1, color: name.trim() ? 'var(--ink)' : 'var(--muted)' }}>
            House of {displayName}
          </div>
        </div>
        ...
        />
        <div style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--muted)', marginTop: 8, textAlign: 'right', letterSpacing: '0.1em' }}>
          {name.length}/24
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: '100dvh', width: '100%', maxWidth: 390, margin: '0 auto', background: 'var(--bg)', display: 'flex', flexDirection: 'column', padding: '60px 24px env(safe-area-inset-bottom, 40px)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
        <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 10, alignSelf: 'flex-start' }}>Step 2 of 4</div>
        ...
        <p style={{ fontFamily: 'var(--sans)', fontSize: 14, color: 'var(--muted)', lineHeight: 1.5, marginBottom: 36, alignSelf: 'flex-start' }}>
          This color marks every territory you control on the map.
        </p>
        ...
        <div style={{
          ...
          fontFamily: 'var(--serif)', fontSize: 42, fontStyle: 'italic',
          marginBottom: 10,
        ...
        <div style={{ fontFamily: 'var(--serif)', fontSize: 16, fontStyle: 'italic', color: 'var(--muted)', marginBottom: 36 }}>
          House of {displayName}
        </div>
            ...
            <button key={c.value} onClick={() => onChange(c.value)} style={{
              ...
              {color === c.value && (
                <svg width="16" height="12" viewBox="0 0 16 12" fill="none">
                  <path d="M1 6L6 11L15 1" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              )}
            </button>
          ))}
        ...
        <div style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.14em', color: 'var(--muted)', marginTop: 14 }}>
          {BANNER_COLORS.find(c => c.value === color)?.label ?? ''}
        </div>
      </div>
...
function StepStartingRegion({ selected, onSelect, onNext }: { selected: number | null; onSelect: (i: number) => void; onNext: () => void }) {
  return (
    <div style={{ minHeight: '100dvh', width: '100%', maxWidth: 390, margin: '0 auto', background: 'var(--bg)', display: 'flex', flexDirection: 'column', padding: '60px 24px env(safe-area-inset-bottom, 40px)' }}>
      ...
      <div style={{ fontFamily: 'var(--mono)', fontSize: 9, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 10 }}>Step 3 of 4</div>
      ...
      <p style={{ fontFamily: 'var(--sans)', fontSize: 14, color: 'var(--muted)', lineHeight: 1.5, marginBottom: 24 }}>
        Choose your starting region. Nearby territories will be available to claim first.
      </p>
        ...
        {STARTING_REGIONS.map((r, i) => {
          ...
          return (
            <button key={i} onClick={() => onSelect(i)} style={{
              ...
              boxShadow: isSelected ? '0 4px 16px rgba(17,17,17,0.15)' : '0 1px 3px rgba(0,0,0,0.04)',
            }}>
              {/* Radio */}
              <div style={{
                width: 20, height: 20, borderRadius: 999, flexShrink: 0,
                border: `2px solid ${isSelected ? '#f4f1ea' : 'var(--line)'}`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
              ...
              <div>
                ...
                <div style={{ fontFamily: 'var(--sans)', fontSize: 12, color: isSelected ? 'rgba(244,241,234,0.55)' : 'var(--muted)', marginTop: 3, lineHeight: 1.4 }}>{r.desc}</div>
                <div style={{ fontFamily: 'var(--mono)', fontSize: 9, color: isSelected ? 'rgba(244,241,234,0.4)' : 'var(--muted)', marginTop: 5, letterSpacing: '0.1em' }}>
                  {r.countries.join(' · ')}
                </div>
              </div>
            </button>
...
/* ─── Step 4: Ready ───────────────────────────────── */
function StepReady({ displayName, color, onEnter }: { displayName: string; color: string; onEnter: () => void }) {
  return (
    <div style={{
      minHeight: '100dvh', width: '100%', maxWidth: 390, margin: '0 auto',
      background: 'var(--ink)', display: 'flex', flexDirection: 'column',
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', gap: 0 }}>
        {/* House badge */}
        <div style={{
          width: 88, height: 88, borderRadius: 999, background: color,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontFamily: 'var(--serif)', fontSize: 44, fontStyle: 'italic', color: '#fff',
          boxShadow: `0 0 0 12px ${color}22, 0 0 0 24px ${color}11`,
        ...
        <div style={{ fontFamily: 'var(--serif)', fontSize: 42, fontStyle: 'italic', letterSpacing: '-0.02em', lineHeight: 1, color: '#f4f1ea', marginBottom: 4 }}>
          House of
        </div>
        <div style={{ fontFamily: 'var(--serif)', fontSize: 42, fontStyle: 'italic', letterSpacing: '-0.02em', lineHeight: 1, color: '#f4f1ea', marginBottom: 32 }}>
          {displayName}
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, width: '100%' }}>
          {[
            { label: 'Territories', value: '0' },
            { label: 'Crown Value', value: '0 pts' },
            { label: 'Rank',        value: 'Unranked' },
          ].map(s => (
            <div key={s.label} style={{
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              padding: '12px 18px', borderRadius: 12,
              background: 'rgba(244,241,234,0.05)', border: '0.5px solid rgba(244,241,234,0.08)',
            }}>
